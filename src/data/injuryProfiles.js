/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
export const injuryKeysByCause = {
  ballistic: [
...
"gsw_abdomen_multiple": {
  ...
  triageLogic: ({ bleeding }) => bleeding ? "Immediate" : "Delayed",
  vitals: ({ bleeding }) => bleeding ? {
    pulse: [130, 145],
    respiratory: [28, 36],
    bp: [65, 85],
    spo2: [85, 90],
...
"facial_laceration_bleeding": {
  ...
  triageLogic: ({ airway }) => airway ? "Immediate" : "Delayed",
  vitals: ({ airway }) => airway ? {
    pulse: [120, 135],
    respiratory: [28, 34],
    bp: [85, 100],
    spo2: [80, 90],
...
"tq_upper_leg_arterial": {
  ...
  triageLogic: ({ arterial }) => arterial ? "Immediate" : "Delayed",
   vitals: ({ arterial }) => arterial ? {
     pulse: [135, 155],
     respiratory: [24, 30],
     bp: [65, 85],
     spo2: [85, 90],
...
"multiple_systems_failure": {
  ...
  "gsw_chest": {
    ...
    requiredInterventions: ["Chest Seal", "Needle Decompression"],
    pneumoChance: 0.5,
    triageLogic: ({ pneumo }) => pneumo ? "Immediate" : "Delayed",
    vitals: ({ pneumo }) => pneumo ? {
      pulse: [130, 145],
      respiratory: [30, 38],
      bp: [80, 95],
      spo2: [75, 88],
  ...
  "head_injury": {
    ...
    amsChance: 0.3,
    triageLogic: ({ ams }) => ams ? "Immediate" : "Delayed",
    vitals: ({ ams }) => ams ? {
      pulse: [110, 130],
      respiratory: [18, 24],
      bp: [90, 110],
...
"blast_concussion_immediate": {
  ...
  requiredInterventions: [],   // no lifesaving device; monitor + evac
  triageLogic: () => "Immediate",
  vitals: () => ({
    pulse: [105, 120],
    respiratory: [20, 24],
...
"frostbite_handsfeet_immediate": {
  ...
  requiredInterventions: [],  // re‑warm, evac; no aid‑bag item needed
  triageLogic: () => "Immediate",
  vitals: () => ({
    pulse: [90, 100],
    respiratory: [18, 22],
